...
class UnitreeUrdfFileCfg(sim_utils.UrdfFileCfg):
    ...
    def replace_asset(self, meshes_dir, urdf_path):
        ...
        """
        tmp_meshes_dir = "/tmp/IsaacLab/loco_lab_main_branch/meshes"
        if os.path.exists(tmp_meshes_dir):
            os.remove(tmp_meshes_dir)
        os.makedirs("/tmp/IsaacLab/loco_lab_main_branch", exist_ok=True)
        os.symlink(meshes_dir, tmp_meshes_dir)

        self.asset_path = "/tmp/IsaacLab/loco_lab_main_branch/robot.urdf"
...
UNITREE_GO2_CFG = UnitreeArticulationCfg(
    # Here we prefer URDF over USD for the convienience of modifying collision shapes
